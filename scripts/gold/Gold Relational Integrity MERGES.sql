...
-- the extras that had multiple values for the same card_id were 
STRING_AGG'd
...
LEFT JOIN silver.card_extras_flattened f ON c.card_id = f.card_id
...
target.subtype_list = source.subtype_list,
target.type_list = source.type_list,
target.resistances_list = source.resistances_list,
--target.resistance_value = source.resistance_value,
target.weaknesses_list = source.weaknesses_list,
--target.weakness_value = source.weakness_value,
target.pokedex_list = source.pokedex_list,
target.small_image_url = source.small_image_url,
target.large_image_url = source.large_image_url


WHEN NOT MATCHED BY TARGET THEN
INSERT (card_key,subtype_list,type_list,resistances_list,weaknesses_list,pokedex_list,small_image_url,large_image_url)
VALUES (source.card_key,source.subtype_list,source.type_list,source.resistances_list,source.weaknesses_list,source.pokedex_list,source.small_image_url,large_image_url);
