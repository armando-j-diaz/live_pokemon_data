...
SELECT 
		cards.*,
	    DATEADD(HOUR,-5,GETDATE()) as load_date --meta data for Historical Analysis --data type: datetime
...
SET @sql_executable = REPLACE(@sql_extract_format,'{{filename}}',CONCAT('pokemon_data_',CAST(FORMAT(DATEADD(HOUR,-5,GETDATE()),'yyyyMMdd') AS NVARCHAR(8)),'.json'))
-- Note that the date here^^^ is given a DATEADD(HOUR,-5) due to the fact that linux Azure Consumption Plans are based on UTC Timezone, and cant be changed.
-- I am in EST, so i did -5, change your accordingly.
...
TRUNCATE TABLE bronze.cards_json_raw
