        ...
        ,Evolutions AS (
          ...
          --  id,
           ...
           -- child.id,
            child.name,
            parent.evolution +1
            FROM Distinct_Pokemons child
            INNER JOIN Evolutions parent
            ON parent.name = child.evolves_from
            )
...
*/
        ...
        SELECT card_id, card_name, supertype, hp, evolution_tier, artist, set_card_rarity, set_card_number,description
        FROM(SELECT DISTINCT
                ...
                RANK() OVER(PARTITION BY card_id ORDER BY e.evolution) as evo_check
            FROM bronze.cards_json_raw c
            LEFT JOIN Evolutions e ON c.name = e.name
